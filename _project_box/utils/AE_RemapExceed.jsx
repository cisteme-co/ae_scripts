...
(function (me) {
	...
	String.prototype.replaceAll = function (s, d) {
		return this.split(s).join(d);
	};

	FootageItem.prototype.nameTrue = function () {
		var b = this.name;
		this.name = '';
		var ret = this.name;
		this.name = b;
	...
	var aeremapCallPath = File.decode('c:/tmp/adobe/AE_RemapCall.exe');
	...
	var findComp = function (op) {
		function makeComp() {
			...
			if (ret != null) {
				var str =
					'このコンポはAE_Remap.jsxによって作られたものです。\r\n' +
	...
	var export_ardj = function () {
		...
		try {
			var r = system.callSystem(
				cmd + ' /SAVE_TO_PATH' + ' "' + ardj.fsName + '"'
			);
			ret = true;
		} catch (e) {
			alert('export_ardj2\r\n' + e.toString());
	...
	var winObj =
		me instanceof Panel
			? me
			: new Window('palette', 'AE_RemapExceed', [0, 0, 250, 330], {
					resizeable: true,
					maximizeButton: true,
					minimizeButton: true,
			  });
	...
	var btnW = 90;
	var btnH = 25;
	var stCaption = winObj.add(
		'statictext',
		[px, py, px + 220, py + 20],
		'AE_Remap Exceed'
	);
	py = 40;
	var btnExec_AERemap = winObj.add(
		'button',
		[px, py, px + btnW, py + btnH],
		'AE_Remap起動'
	);
	py += 30;
	var btnGetClip = winObj.add(
		'button',
		[px, py, px + btnW, py + btnH],
		'セル情報獲得'
	);
	py += 30;
	var btnSaveArdj = winObj.add(
		'button',
		[px, py, px + btnW, py + btnH],
		'ardj保存'
	);
	py += 30;
	var btnToComp = winObj.add(
		'button',
		[px, py, px + btnW, py + btnH],
		'compへ保存'
	);
	py += 30;
	var btnFromComp = winObj.add(
		'button',
		[px, py, px + btnW, py + btnH],
		'compから読込'
	);
	...
	py += 30;
	var btnCenter = winObj.add(
		'button',
		[px, py, px + btnW, py + btnH],
		'Center'
	);
	py += 30;
	var btnGetCellLayer = winObj.add(
		'button',
		[px, py, px + btnW, py + btnH],
		'getCellLayer'
	);
	py += 30;
	var cmbKoma = winObj.add(
		'dropdownlist',
		[px, py, px + btnW, py + btnH],
		['ディゾルブ', 'リマップ最大値', '不透明度']
	);
	...
	px = 110;
	py = 40;
	var edInfo = winObj.add('edittext', [px, py, px + 150, py + 25], '', {
		readonly: true,
	});
	py += 30;
		...
		if (ary.length > 0) {
			...
			for (var i = 0; i < ary.length; i++) {
				...
				p.onClick = function () {
					...
					btnApply.text =
						this.idx + ': ' + cellData.caption[this.idx] + 'を適応';
				};
				rbtns.push(p);
				y += 23;
			}
	...
	var getCellLayer = function () {
		...
		try {
			var r = system.callSystem(
				cmd + ' /import_layer "' + saveFile.fsName + '"'
			);
		} catch (e) {
