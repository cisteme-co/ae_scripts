...
function renderBG() {
	...
	if (rq.numItems > 0) {
		...
		// Ensure every queued item has an mp4 output module; add one if missing
		for (var ii = 1; ii <= rq.numItems; ii++) {
			var qItem = rq.item(ii);
			if (qItem.status != RQItemStatus.QUEUED) continue;
			var hasMp4 = false;
			for (var jj = 1; jj <= qItem.numOutputModules; jj++) {
				var omFile = qItem.outputModule(jj).file;
				if (omFile && omFile.name.split('.').pop().toLowerCase() === 'mp4') {
					hasMp4 = true;
					break;
				}
			}
			if (!hasMp4) {
				...
				newOM.file = new File(
					mp4Folder.fsName + '/' + qItem.comp.name + '.mp4',
				);
				rqOK = true;
			}
		}
