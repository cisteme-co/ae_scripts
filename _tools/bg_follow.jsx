...
if (isValid(app.project.activeItem) == true) {
    ...
    for (var i = 0; i < selectedLayers.length; i++) {
        ...
        loop.property("ADBE Offset-0001").expression = "var offsetX = thisLayer.width/2;var offsetY = thisLayer.height/2;\n" +
            "var fps= 1/thisComp.frameDuration;\n" +
            "(posterizeTime(fps/effect('Frame Frequency')('スライダー')));\n" +
            "var followX =0;\n" +
            "var followY =0;\n" +
            "for(var f=0;f<Math.round(time/thisComp.frameDuration);f++){\n" +
            "var DPM=effect('Resolution (in DPI)')('スライダー').valueAtTime(f*thisComp.frameDuration)/25.40;\n" +
            "var _shift=effect('Speed (mm/k)')('スライダー').valueAtTime(f*thisComp.frameDuration);\n" +
            "var _angle = effect('Angle')('角度').valueAtTime(f*thisComp.frameDuration) + 90;\n" +
            "if (effect('Opposite')('チェックボックス').valueAtTime(f*thisComp.frameDuration) == true) {FollowFlag = 'SLIDE'} else {FollowFlag = 'FOLLOW'}\n" +
        ...
        noLoop.property("ADBE Geometry2-0001").expression = "[thisLayer.width/2, thisLayer.height/2]";
        noLoop.property("ADBE Geometry2-0002").expressionEnabled = true;
        noLoop.property("ADBE Geometry2-0002").expression = "var offsetX = thisLayer.width/2;var offsetY = thisLayer.height/2;\n" +
            "var fps= 1/thisComp.frameDuration;\n" +
            "(posterizeTime(fps/effect('Frame Frequency')('スライダー')));\n" +
            "var followX =0;\n" +
            "var followY =0;\n" +
